...
Startup sequence:
  ...
  4. Start DB listener background task  (Postgres → Redis)
  5. Start Redis broadcaster background task (Redis → WebSockets)
...
from db_listener        import start_db_listener, total_events_fired
from models             import (
...
async def lifespan(app: FastAPI):
    ...
    # 4. Background: Postgres → Redis
    listener_task = asyncio.create_task(
        start_db_listener(redis_client),
        name="db_listener",
    )

    # 5. Background: Redis → WebSockets (all portals)
...
async def metrics():
    ...
    return MetricsResponse(
        ...
        total_events_fired = db_listener.total_events_fired,
